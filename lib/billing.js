 ...
 * Ultimate is differentiated by team seats, SSO, swarm execution,
 * industry context, and a larger Business Assistant message allowance
 * — see swarmSlots/teamSeats/sso/businessAssistantMessagesPerMonth
 * below. If you're marketing these two side by side, the name overlap
 * ("Unlimited" and "Ultimate" both implying infinity) is worth
 * revisiting before launch.
 *
 * businessAssistantMessagesPerMonth is a plan-config number only —
 * WHETHER a plan gets Business Assistant at all is enforced separately
 * by auth.js's requireBusinessAssistant (unlimited/ultimate only; free
 * and pro are blocked with a 402 upgrade nudge before ever reaching a
 * route that would check this number). Actually counting a user's
 * messages against this monthly figure and blocking past it is NOT
 * wired up yet — no usage counter exists for it today. Enforcement is
 * a real, separate follow-up (same real usage-tracking work already
 * planned for AI token/image/video usage), not implemented here.
 *
...
const PLANS = {
  free: {
    priceId: null,
    buildsPerMonth: 3, maxProjects: 1, swarmSlots: 1, teamSeats: 1, sso: false, industryContext: false, whiteLabel: false,
    businessAssistantMessagesPerMonth: 0 // no Business Assistant access at all — see auth.js's requireBusinessAssistant
  },
  pro: {
    ...
    businessAssistantMessagesPerMonth: 0 // no Business Assistant access — auth.js's requireBusinessAssistant blocks Pro with a 402 upgrade nudge
  },
  unlimited: {
    priceId: process.env.STRIPE_PRICE_UNLIMITED, // £79.99/mo
    buildsPerMonth: Infinity, maxProjects: Infinity, swarmSlots: 2, teamSeats: 1, sso: false, industryContext: false,
    whiteLabel: true,       // strips the "Powered by Gurost" badge — see note in web-bot.js/variant-bot.js
    businessAssistantMessagesPerMonth: 500
  },
  ultimate: {
    priceId: process.env.STRIPE_PRICE_ULTIMATE, // £99/mo, fixed — not contact-sales
    ...
    businessAssistantMessagesPerMonth: 2000
  }
};
...
// Real, same pattern as createCheckoutSession above - server.js calls
// this rather than reaching for the raw stripe client directly, which
// isn't exported from this module (and shouldn't be - every real
// Stripe interaction stays owned here, in one place).
