...
const { getApiKeys } = require("../api-key-vault");
...
async function deployApp(project, projectId) {
  ...
  try {
    ...
    // Real, stored user-provided keys (Stripe, Twilio, whatever this
    // specific app actually needs) - fetched from the real, encrypted
    // Vault storage, injected the same real way DATABASE_URL already
    // is. A key the user never provided just won't be in this list -
    // the deployed app would fail at runtime using it, same real
    // consequence as forgetting to set any other required env var.
    try {
      const userKeys = await getApiKeys(projectId);
      for (const [key, value] of Object.entries(userKeys)) {
        envVars.push({ key, value });
      }
    } catch (err) {
      console.error(`[deploy] Failed to fetch real stored API keys for project ${projectId}, deploying without them:`, err.message);
    }

    const backendDeploy = await deployBackend(repoUrl, projectId, { envVars });
    results.backend = { url: backendDeploy.url, repoUrl, status: "deployed", databaseConnected: envVars.some((v) => v.key === "DATABASE_URL") };
  } catch (err) {
