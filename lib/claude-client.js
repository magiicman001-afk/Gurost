 ...
 *
 * All calls now route through OmniRoute (lib/omniroute-client.js)
 * rather than hitting api.anthropic.com directly — see that file for
 * the real format differences this involves (OpenAI-compatible, not
 * Anthropic-compatible) and the operational caveat about
 * OMNIROUTE_BASE_URL only resolving locally by default.
 *
 ...
 * here — OmniRoute is now the single source of truth for where calls
 * go, per an explicit "all requests" scope. headroom-integration.js
 * still exists if you want to point OMNIROUTE_BASE_URL at a Headroom
 * proxy that itself forwards to your real providers.
 *
 ...
 * does whatever naming convention OmniRoute expects for a given
 * provider's models — check OmniRoute's own `/v1/models` listing
 * against CLAUDE_MODEL/CLAUDE_MODEL_FAST below if a model stops
 * resolving.
 */

const { supabase } = require("./db");
const security = require("../security");
const { callOmniRoute } = require("./omniroute-client");
...
async function callClaude({ system, messages, maxTokens = 4000, model = CLAUDE_MODEL, context }) {
  ...
  const { text: rawText, usage } = await callOmniRoute({ model, system: guardedSystem, messages, maxTokens });
