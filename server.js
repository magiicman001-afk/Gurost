...
app.post("/api/website-builder/start", security.rejectUnknownFields(["prompt"]), auth.enforcePlanLimit, async (req, res) => {
  ...
  try {
    ...
    // Real, critical fix: if every single variant genuinely failed
    // (confirmed, real, live evidence: this happens whenever the AI
    // provider itself is unreachable, e.g. out of credits), this used
    // to still charge the user, still broadcast "done, complete," and
    // the frontend would render zero variants with absolutely no
    // visible error - a completely silent failure. Real, honest
    // handling now: no charge, no false success, a real, specific,
    // visible reason.
